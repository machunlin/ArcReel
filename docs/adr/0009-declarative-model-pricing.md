...
- **改促销价 = 改声明 + 重部署**：这是 conscious trade-off。代价是不支持非技术人员在运行时改价；收益是无需 DB schema/迁移/仓库/设置页 UI/三语 i18n/校验这一整套 surface。AI漫剧 自托管、重部署是常态，且成本仅用于费用预估与内部记账（非对外计费），精度诉求是"不离谱"而非"实时可调"。
